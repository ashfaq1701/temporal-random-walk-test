...
def filter_oov_nodes(train_ids, train_y, val_ids, val_y, test_ids, test_y, embedding_dict, max_node_id):
    """
    Remove IDs that don't have a pretrained embedding.
    Returns filtered splits and a small report dict.
    """
    # valid nodes are those with an embedding and within [0..max_node_id]
    valid = np.fromiter((nid for nid in embedding_dict.keys() if nid <= max_node_id), dtype=np.int32)
    valid.sort()  # np.isin is faster with sorted 'test_elements'

    def _filter(ids, y):
        mask = np.isin(ids, valid, assume_unique=False)
        return ids[mask], y[mask], mask

    tr_ids, tr_y, tr_mask = _filter(train_ids, train_y)
    va_ids, va_y, va_mask = _filter(val_ids,   val_y)
    te_ids, te_y, te_mask = _filter(test_ids,  test_y)

    report = {
        "train_kept": int(tr_mask.sum()), "train_dropped": int((~tr_mask).sum()),
        "val_kept":   int(va_mask.sum()), "val_dropped":   int((~va_mask).sum()),
        "test_kept":  int(te_mask.sum()), "test_dropped":  int((~te_mask).sum()),
    }
    return tr_ids, tr_y, va_ids, va_y, te_ids, te_y, report
...
def run_bot_detection_experiments(
    ...
    train_ids_filtered, train_labels_filtered, val_ids_filtered, val_labels_filtered, test_ids_filtered, test_labels_filtered, rep = filter_oov_nodes(
        train_ids, train_labels, val_ids, val_labels, test_ids, test_labels,
        embedding_dict=embeddings, max_node_id=max_node_id
    )
    logger.info(
        f"After dropping OOV: train {rep['train_kept']:,} (-{rep['train_dropped']:,}), "
        f"val {rep['val_kept']:,} (-{rep['val_dropped']:,}), "
        f"test {rep['test_kept']:,} (-{rep['test_dropped']:,})"
    )
