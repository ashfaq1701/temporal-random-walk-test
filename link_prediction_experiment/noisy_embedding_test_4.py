...
def train_embeddings_streaming_approach(
    ...
    walk_use_gpu, batch_epochs, seed=42,
    # optional knobs (safe defaults)
    base_alpha=0.03,             # per-batch starting LR
    min_alpha_ratio=0.1,         # end_alpha = base_alpha * this
    negative=10,                 # #neg samples (0 ⇒ HS)
    sample=1e-4,                 # subsampling of frequent tokens; 0 disables
    shrink_windows=False,        # keep full window each epoch
    replay_batches=2,            # how many past batches to replay
    replay_ratio=0.25,           # fraction of last-batch walks to keep in replay
    batch_words=50_000           # internal chunking in gensim.train()
):
    """
    Incremental Word2Vec over time batches, with order-preserving replay.

    Note: for strict temporal update order, set word2vec_n_workers=1 (parallel workers
    process sentences out of order).
    """
    logger.info("Training embeddings with streaming approach (incremental Word2Vec)")

    # ---- helper: re-iterable (multi-epoch) corpus wrapper ----
    class _ReIterableCorpus:
        """Wraps replay + current batch so gensim can iterate fresh each epoch."""
        def __init__(self, replay_slices, current_batch):
            # snapshot (no deep copy of sentences)
            self._replay = list(replay_slices)  # list of lists-of-sentences
            self._current = current_batch       # list-of-sentences
        def __iter__(self):
            from itertools import chain
            return chain(*self._replay, self._current) if self._replay else iter(self._current)
        def __len__(self):
            return sum(len(x) for x in self._replay) + len(self._current)
    ...
    num_batches = int(np.ceil(total_range / batch_ts_size)) if batch_ts_size > 0 else 0
    logger.info(f"Processing {num_batches} batches with duration={batch_ts_size:,}")

    w2v_model = None
    replay = deque(maxlen=max(0, int(replay_batches)))  # holds recent slices (lists-of-sentences)

    for batch_idx in range(num_batches):
        batch_start_ts = min_ts + batch_idx * batch_ts_size
        batch_end_ts   = min_ts + (batch_idx + 1) * batch_ts_size
        if batch_idx == num_batches - 1:
            batch_end_ts = max_ts + 1  # include tail
        ...
        # ---- update temporal graph & generate walks (forward + backward) ----
        ...
        del walks, lens, walks_f, walks_b, lens_f, lens_b
        ...
        try:
            with suppress_word2vec_output():
                # ---- (re)build vocab (only the *new* tokens) ----
                if w2v_model is None:
                    hs_flag = 0 if (negative and negative > 0) else 1
                    w2v_model = Word2Vec(
                        ...
                        sample=float(sample),
                        shrink_windows=bool(shrink_windows),
                        seed=int(seed)
                    )
                    w2v_model.build_vocab(clean_walks)
                else:
                    # only extend vocab with tokens from current batch (no replay here)
                    w2v_model.build_vocab(clean_walks, update=True)

                # ---- chronological replay (no shuffling) ----
                if replay and replay_ratio > 0:
                    n_keep = max(1, int(len(clean_walks) * float(replay_ratio)))
                    recent_slice = clean_walks[-n_keep:]
                else:
                    recent_slice = clean_walks

                corpus = _ReIterableCorpus(replay, clean_walks)
                total_examples = len(corpus)  # sentences per epoch

                # ---- per-batch learning rate schedule ----
                start_alpha = float(base_alpha)
                end_alpha   = float(base_alpha) * float(min_alpha_ratio)
                ...
                w2v_model.train(
                    ...
                    end_alpha=end_alpha,
                    compute_loss=False,
                    batch_words=int(batch_words)
                )

                # update replay buffer
                if replay.maxlen and replay_ratio > 0:
                    replay.append(recent_slice)

        except Exception as e:
            logger.error(f"Error processing batch {batch_idx + 1}: {e}")
            continue

        # free batch lists to keep memory steady
        del clean_walks
    ...
    # ---- extract & L2-normalize embeddings once at the end ----
