...
def train_embeddings_streaming_approach(train_sources, train_targets, train_timestamps,
                                        batch_ts_size, sliding_window_duration, time_decay_lambda,
                                        is_directed, walk_length, num_walks_per_node, edge_picker,
                                        embedding_dim, walk_use_gpu, word2vec_n_workers, seed=42):
    logger.info("Training embeddings with streaming approach (time decay)")
    ...
    for batch_idx in range(num_batches):
        ...
        try:
            ...
            # Compute batch time as the last timestamp
            batch_time = batch_ts.max()

            # Merge with global embeddings using time-decay
            for node_id, batch_embedding in batch_embeddings.items():
                batch_embedding = np.array(batch_embedding)

                if node_id in global_embeddings:
                    last_time = last_update_time[node_id]
                    delta_t = batch_time - last_time
                    alpha = np.exp(-time_decay_lambda * delta_t)

                    global_embeddings[node_id] = (
                            alpha * global_embeddings[node_id] +
                            (1 - alpha) * batch_embedding
                    )
                else:
                    # First time seeing this node
                    global_embeddings[node_id] = batch_embedding

                last_update_time[node_id] = batch_time
...
if __name__ == '__main__':
    ...
    # Model parameters
    parser.add_argument('--time_decay_lambda', type=float, default=0.05,
                        help='Decay rate λ for time-based embedding updates (higher = faster forgetting)')
